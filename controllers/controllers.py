...
class BusinessTripRedirect(http.Controller):
    ...
    def redirect_user_by_role(self, **kwargs):
        ...
        # Regular users → go to their forms list
        # کاربران عادی به لیست فرم‌های خودشان هدایت می‌شوند
    ...
    def create_new_trip_form(self, sale_order_id, **kwargs):
        """ایجاد یک فرم جدید صرف نظر از وجود فرم‌های قبلی"""
        # Get the target quotation
        sale_order = request.env['sale.order'].sudo().browse(sale_order_id)
        if not sale_order.exists():
            return request.not_found()
        ...
        # ایجاد فرم جدید بدون بررسی فرم‌های قبلی
        form = request.env['formio.form'].sudo().create({
            'builder_id': builder.id,
    ...
    def create_standalone_trip_form(self, **kwargs):
        """ایجاد یک فرم سفر بدون ارتباط با کوتیشن"""
        # Get the builder first
        builder = request.env['formio.builder'].sudo().search([
            ('state', '=', 'CURRENT'),
            ('res_model_id.model', '=', 'sale.order')  # همین بیلدر را استفاده می‌کنیم
        ], limit=1)

        if not builder:
            return request.not_found('custom_business_trip_management.template_no_builder')

        # ایجاد فرم جدید بدون ارتباط با کوتیشن
        form = request.env['formio.form'].sudo().create({
            'builder_id': builder.id,
